 ...
 * By default, validates date-times in ISO format (yyyy-MM-dd'T'HH:mm:ss).
 */
@Documented
...
public @interface ValidDateTime {
     ...
     * Default is ISO local date-time format (yyyy-MM-dd'T'HH:mm:ss).
     ...
     * Default is empty, meaning there is no minimum.
     */
    String min() default "";

    /**
     * Specifies the maximum date-time (inclusive) in the same format as the pattern.
     * Default is empty, meaning there is no maximum.
     */
    String max() default "";
